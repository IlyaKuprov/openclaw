...
describe("OpenAI Responses SDK transport accounting", () => {
  it("executes the managed ChatGPT route with physical dispatch accounting", async () => {
  ...
  it("retains Azure dispatch accounting from a legacy-only attested host", async () => {
    const events: AiModelTransportEvent[] = [];
    configureTransportObserver(events);
    configureAiTransportHost({
      ...getAiTransportHost(),
