...
describe("OpenAI native transport accounting", () => {
  it("uses native SSE header authority and nested event-header precedence", async () => {
    ...
    expect(dispatchEvents(events)).toMatchObject([
      {
  ...
  it("does not fallback when a partial-authority WebSocket send rejection is unverified", async () => {
    const events: AiModelTransportEvent[] = [];
    configureAttestedTransportObserver(events);
  ...
  it("does not fallback after an admitted WebSocket send can have reached the provider", async () => {
    const events: AiModelTransportEvent[] = [];
    configureAttestedTransportObserver(events);
