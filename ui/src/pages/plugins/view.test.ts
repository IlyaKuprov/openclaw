...
describe("renderPlugins", () => {
  ...
  it("renders install policy findings with cancel and acknowledged retry actions", () => {
    ...
    const row = container.querySelector<HTMLElement>('[data-plugin-id="kitchen-sink"]');
    const alert = row?.querySelector('[role="alert"]');
    expect(normalizedText(alert)).toContain("Security review needed");
    expect(normalizedText(alert)).toContain("ClawScan found 1 warning");
    expect(normalizedText(alert)).toContain("This plugin has not been installed");
    expect(normalizedText(alert)).toContain("What ClawScan found");
    expect(normalizedText(alert)).toContain("Semgrep found a risky command.");
    const technicalDetails = alert?.querySelector<HTMLDetailsElement>(
