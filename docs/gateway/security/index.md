...
- Installing/updating plugins runs executable code:
  ...
  - A `warn` result stops before commit. Review it, then rerun the CLI install with `--dangerously-force-unsafe-install`; OpenClaw re-evaluates policy and only acknowledges another `warn`. `block` and policy failures remain terminal.
