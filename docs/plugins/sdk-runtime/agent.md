...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    Direct imports from `openclaw/plugin-sdk/session-store-runtime` cannot mutate or clean internal-effects sessions; use `api.runtime.agent.session.patchSessionEntry(...)` and `api.runtime.agent.session.cleanupSessionLifecycleArtifacts(...)` for owned children so the host binds the calling plugin to writes and cleanup.
