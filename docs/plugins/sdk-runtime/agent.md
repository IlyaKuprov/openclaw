...
<AccordionGroup>
  <Accordion title="api.runtime.agent">
    ...
    **Session store helpers** are under `api.runtime.agent.session`:
    ...
    Direct imports from `openclaw/plugin-sdk/session-store-runtime` cannot mutate or clean internal-effects sessions; use `api.runtime.agent.session.patchSessionEntry(...)` and the optional `api.runtime.agent.session.cleanupSessionLifecycleArtifacts(...)` capability for owned children so the host binds the calling plugin to writes and cleanup. `PluginRuntimeWithSessionLifecycleCleanupV1` in `openclaw/plugin-sdk/core` requires the capability; the legacy `PluginRuntime` shape keeps it optional for older external runtime adapters. Check that cleanup is present before creating a lifecycle-owned child and refuse the operation if absent rather than using the direct SDK cleanup helper, which lacks owner-bound authority.
