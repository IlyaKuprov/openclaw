...
export function frontierTraceFailure(
  ...
  const receipt = trace.source.dispatchReceipt;
  const persistedReceipt = receipt
    ? normalizeAgentExecDispatchReceipt(JSON.stringify(receipt))
    : undefined;
  if (
    ...
    !receipt ||
    !persistedReceipt ||
    !verifyAgentExecDispatchReceipt(JSON.stringify(receipt)) ||
    !receipt.complete ||
    receipt.truncated ||
...
export function buildFrontierAuditBundle(params: {
  ...
  const fieldDigests = {
    dispatchReceiptSha256: digestFrontierEvidence(
      "frontier.dispatch-receipt.v2",
      persistedTrace.source.dispatchReceipt,
    ),
...
function executionMatchesPlan(
  ...
  return (
    ...
    execution.source.sourceDirty === false &&
...
export function verifyFrontierAuditBundle(
  ...
  const trace = normalizeAgentExecTrace(traceJson);
  const receiptJson = JSON.stringify(bundle.trace.source.dispatchReceipt);
  const receipt = normalizeAgentExecDispatchReceipt(receiptJson);
  if (
    !trace ||
    !receipt ||
    !verifyAgentExecTrace(traceJson) ||
    !verifyAgentExecDispatchReceipt(receiptJson) ||
    canonicalJson(trace) !== canonicalJson(bundle.trace) ||
    canonicalJson(receipt) !== canonicalJson(bundle.trace.source.dispatchReceipt) ||
    bundle.digests.dispatchReceiptSha256 !==
      digestFrontierEvidence("frontier.dispatch-receipt.v2", receipt) ||
