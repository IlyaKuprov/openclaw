...
export function terminalZeroSubmissionTrace() {
  const source = snapshot("code");
  const callId = "zero-call";
  const collector = createProviderTransportAccountingCollector();
  collector.observer.onLogicalCallStarted({
    callId,
    provider: "openai",
  ...
  if (!projected.snapshot || projected.coverage.state !== "complete") {
    throw new Error("expected zero submission fixture");
  }
