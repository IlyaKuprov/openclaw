...
describe("v2 envelope authority", () => {
  it("uses public v2 normalize/verify and source/projection paths after persistence", () => {
    ...
    expect(normalized).toMatchObject({
      ...
      source: {
        ...
        dispatchReceipt: { complete: true, truncated: false, incompleteReasons: [] },
      },
      projection: {
        metrics: {
  ...
  it("rejects persisted zero-submission proof loss", () => {
    const zeroTrace = terminalZeroSubmissionTrace();
    const persisted = JSON.stringify(zeroTrace);
    expect(normalizeAgentExecTrace(persisted)?.audit).toMatchObject({
      state: "inconclusive",
      reasons: expect.arrayContaining(["dispatch_receipt_producer_proof_not_persisted"]),
    });
    const value = JSON.parse(envelope("code"));
    value.trace = zeroTrace;
    const normalized = frozenFrontierMatrixTesting.normalizeEnvelope(JSON.stringify(value));
    expect(normalized?.trace.audit).toMatchObject({
      state: "inconclusive",
      reasons: expect.arrayContaining(["dispatch_receipt_producer_proof_not_persisted"]),
    });
  });
...
describe("artifact safety", () => {
  it.each([
    ...
    [{ nested: { value: `sk-${"test-secret-value"}` } }, "sensitive text"],
