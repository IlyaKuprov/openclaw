...
async function handleSlackPinEvent(params: {
  ...
  const { ctx, trackEvent, body, context, client, event, action, contextKeySuffix, errorLabel } =
    params;
  ...
  try {
    ...
    enqueueSystemEvent(
      `Slack: ${userLabel} ${action} a ${itemType} in ${ingressContext.channelLabel}.`,
      {
        ...
        contextKey: `slack:pin:${eventScope ? `${eventScope.teamId}:` : ""}${contextKeySuffix}:${channelId ?? "unknown"}:${messageId ?? "unknown"}`,
      },
    );
  } catch (err) {
