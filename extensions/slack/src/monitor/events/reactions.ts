...
export function registerSlackReactionEvents(params: {
  ...
  const handleReactionEvent = async (
    ...
    try {
      ...
      enqueueSystemEvent(text, {
        ...
        contextKey: `slack:reaction:${eventScope ? `${eventScope.teamId}:` : ""}${action}:${item.channel}:${item.ts}:${event.user}:${emojiLabel}`,
      });
    } catch (err) {
  ...
  ctx.app.event(
    ...
    async (args: SlackEventMiddlewareArgs<"reaction_added"> & AllMiddlewareArgs) => {
      ...
      await handleReactionEvent(event as SlackReactionEvent, "added", eventScope);
    },
  );
  ...
  ctx.app.event(
    ...
    async (args: SlackEventMiddlewareArgs<"reaction_removed"> & AllMiddlewareArgs) => {
      ...
      await handleReactionEvent(event as SlackReactionEvent, "removed", eventScope);
    },
  );
}
