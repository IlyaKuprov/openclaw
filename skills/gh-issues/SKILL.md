...
For review, read `gh pr view <n> --repo "$SOURCE_REPO" --json headRefOid,isDraft,comments,reviews`, `gh api "repos/$SOURCE_REPO/issues/<n>/comments"`, and `gh api "repos/$SOURCE_REPO/pulls/<n>/comments"` (inline findings and locations). The reviewer is `chatgpt-codex-connector`; its `<!-- codex-pull-request-review-summary -->` issue comment records review status and reviewed commit. If the PR is a draft, mark it ready before expecting review. If there is no review of the current head, request `@codex review` with `gh pr comment`, then wait. Address every actionable finding, including inline threads (reply with fix and commit, or measured reason to reject), push and request review again after each head change. Do not re-request after a clean verdict on the same head. Finish only when the newest review covers the current `headRefOid` with no outstanding findings; a pending review or an opened PR is interim.
