...
function validReceipt(): AgentExecDispatchReceipt {
  const receipt = sealAgentExecDispatchReceipt({
    complete: true,
    truncated: false,
    incompleteReasons: [],
...
function resealReceipt(receipt: AgentExecDispatchReceipt): void {
  const { sha256: _sha256, ...contents } = receipt;
  receipt.sha256 = domainDigest("openclaw.agent-exec.dispatch-receipt.v2", contents);
}
...
function resealAll(trace: AgentExecTrace): void {
  if (trace.source.dispatchReceipt) {
    resealReceipt(trace.source.dispatchReceipt);
  }
  resealSource(trace);
  resealTrace(trace);
}
...
describe("agent exec trace schema v2", () => {
  it("keeps integrity construction out of the public verification module", () => {
    ...
    expect(publicSchema).not.toHaveProperty("sealAgentExecDispatchReceipt");
    expect(validTrace().source).toMatchObject({
      kind: "agent_exec_source_facts",
      dispatchReceipt: { kind: "transport_dispatch_receipt" },
    });
    expect(validTrace().source).not.toHaveProperty("authority");
    expect(validTrace().source.dispatchReceipt).not.toHaveProperty("authority");
  });
  ...
  it("accepts concurrent interleaving but rejects order and lifecycle repair", () => {
    ...
    expect(verifyAgentExecDispatchReceipt(persist(receipt))).toBe(true);

    for (const dispatches of [
      [receipt.dispatches[1]!, receipt.dispatches[0]!, receipt.dispatches[2]!],
      [
        { ...receipt.dispatches[2]!, sequence: 1 },
        receipt.dispatches[1]!,
        { ...receipt.dispatches[0]!, sequence: 3 },
      ],
      [
      ...
      forged.dispatches = dispatches;
      resealReceipt(forged);
      expect(normalizeAgentExecDispatchReceipt(persist(forged))).toBeUndefined();
    }
    ...
    const reopenedContents: AgentExecDispatchReceiptContents = {
      ...receiptContents,
      dispatches: [
        receipt.dispatches[0]!,
        receipt.dispatches[1]!,
    ...
    expect(sealAgentExecDispatchReceipt(reopenedContents)).toBeUndefined();
  });
  ...
  it("rejects noncanonical and unknown reason arrays", () => {
    const incomplete = sealAgentExecDispatchReceipt({
      complete: false,
      truncated: true,
      incompleteReasons: ["provider_events_truncated", "dispatch_ledger_incomplete"],
      logicalCalls: 0,
    ...
    expect(verifyAgentExecDispatchReceipt(persist(noncanonical))).toBe(false);

    const unknown = structuredClone(incomplete!);
    unknown.incompleteReasons = ["looks_valid_but_unknown"];
    resealReceipt(unknown);
    expect(verifyAgentExecDispatchReceipt(persist(unknown))).toBe(false);
  ...
  it("enforces numeric, collection, and byte-size caps", () => {
    ...
    const maxCalls = sealAgentExecDispatchReceipt({
      complete: false,
    ...
    expect(verifyAgentExecDispatchReceipt(persist(oneExtraCall))).toBe(false);

    const maxDispatches = sealAgentExecDispatchReceipt({
      complete: true,
      incompleteReasons: [],
      truncated: false,
      route: { provider: "openai", model: "openai/gpt-5.6", api: "responses" },
      ...
      dispatches: Array.from({ length: 128 }, (_, index) => ({
        sequence: index + 1,
        logicalCallOrdinal: 1,
        perCallAttemptOrdinal: 1,
        hopOrdinal: index + 1,
    ...
    expect(maxDispatches).toBeDefined();
    const oneExtraDispatch = structuredClone(maxDispatches!);
    oneExtraDispatch.dispatches.push({
      sequence: 129,
      logicalCallOrdinal: 1,
      perCallAttemptOrdinal: 1,
      hopOrdinal: 129,
    ...
    oneExtraDispatch.modelFacingApiCalls = 129;
    resealReceipt(oneExtraDispatch);
    expect(verifyAgentExecDispatchReceipt(persist(oneExtraDispatch))).toBe(false);
  ...
  it("rejects digest replay and all receipt digest tampering", () => {
    ...
    expect(verifyAgentExecDispatchReceipt(persist(receipt))).toBe(false);

    for (const mutate of [
      (value: AgentExecDispatchReceipt) => {
      ...
      const forged = structuredClone(trace.source.dispatchReceipt!);
      mutate(forged);
      expect(verifyAgentExecDispatchReceipt(persist(forged))).toBe(false);
    }
  });
  ...
  it("persists normalized output without inherited serialization hooks", () => {
    ...
    expect(verifyAgentExecDispatchReceipt(normalized!.source.dispatchReceipt)).toBe(true);
    expect(verifyAgentExecTrace(serialized)).toBe(true);
    expect(Object.getPrototypeOf(normalized!)).toBeNull();
    expect(Object.isFrozen(normalized)).toBe(true);
    expect(Array.isArray(normalized!.source.dispatchReceipt!.dispatches)).toBe(true);
    expect(Object.isFrozen(normalized!.source.dispatchReceipt!.dispatches)).toBe(true);
    expect(
      Object.getOwnPropertyDescriptor(normalized!.source.dispatchReceipt!.dispatches, "toJSON"),
  ...
  it("never escapes on hostile proxy traps or bounded-shape fuzz cases", () => {
    ...
    for (let iteration = 0; iteration < 64; iteration += 1) {
      ...
      switch (mutationKind) {
        ...
        case 3:
          if ((next() & 1) === 0) {
            trace.source.dispatchReceipt!.dispatches.reverse();
          } else {
            trace.source.dispatchReceipt!.dispatches.push(
              trace.source.dispatchReceipt!.dispatches.shift()!,
            );
          }
          resealAll(trace);
          candidate = persist(trace);
        ...
        case 7:
          ...
          break;
        case 8:
          trace.source.dispatchReceipt!.sha256 = next().toString(16).padStart(64, "0");
          resealSource(trace);
          resealTrace(trace);
          candidate = persist(trace);
          break;
        ...
        case 10:
          ...
          break;
        case 11: {
          const call = trace.source.dispatchReceipt!.calls[next() % 2]!;
          if ((next() & 1) === 0) {
            call.ordinal += 1;
          } else {
