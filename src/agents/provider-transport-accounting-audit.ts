...
function observeTransportTruncation(snapshot: unknown): boolean {
  ...
  const dispatches = readOwnDataProperty(snapshot, "dispatches");
  ...
  return (
    ...
    (readArrayLength(readOwnDataProperty(attempts, "entries")) ?? 0) >
      MAX_MODEL_TRANSPORT_ATTEMPTS ||
    readOwnDataProperty(dispatches, "entriesTruncated") === true ||
    (readArrayLength(readOwnDataProperty(dispatches, "entries")) ?? 0) >
      MAX_MODEL_TRANSPORT_DISPATCHES ||
...
export function extractAuditableProviderTransportAccountingSnapshot(
  ...
  try {
    ...
    const dispatches = normalizedSnapshot.dispatches;
    ...
    if (
      ...
      dispatches.totalKind !== "exact" ||
      dispatches.entriesTruncated ||
      dispatches.total !== dispatches.entries.length ||
      dispatches.entries.length > MAX_MODEL_TRANSPORT_DISPATCHES ||
