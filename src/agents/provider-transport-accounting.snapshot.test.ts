...
describe("extractAuditableProviderTransportAccountingSnapshot", () => {
  ...
  it("rejects duplicate identities and mutated dispatch, attempt, or hop ordinals", () => {
    const fixture = retryFixture();
    const mutations: Array<(snapshot: ProviderTransportAccountingSnapshot) => void> = [
      ...
      (snapshot) => {
        snapshot.dispatches!.entries[1]!.sequence = 3;
      },
      (snapshot) => {
        snapshot.dispatches!.entries[2]!.hopOrdinal = 3;
      },
      (snapshot) => {
