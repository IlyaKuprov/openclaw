...
const SESSION_LIFECYCLE_ERROR_MAX_CHARS = 160;
...
function lifecycleToolResult(result: SessionLifecycleResult, index: number) {
  return {
    ...
    ...(result.error
      ? { error: truncateUtf16Safe(result.error, SESSION_LIFECYCLE_ERROR_MAX_CHARS) }
      : {}),
...
function lifecycleToolEnvelope(
  ...
  if (!sessionsToolResultFitsBudget(payload)) {
    throw new Error("Sessions lifecycle result exceeded its bounded tool contract");
  }
  return payload;
}
