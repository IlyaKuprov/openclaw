...
import {
  ...
  retainProviderTransportDispatch,
...
import {
  ...
  rejectTransportDispatchRelation,
...
function applyAttempt(
  ...
  const pendingDispatch = call.pendingDispatchAttempt;
  if (
    pendingDispatch &&
    (pendingDispatch.ordinal !== event.ordinal || pendingDispatch.reason !== event.reason)
  ) {
    return rejectTransportDispatchRelation(state);
  }
  ...
  if (!pendingDispatch) {
    state.issues.add("transport_dispatch_relation_incomplete");
    state.aggregateLowerBounds.dispatches = true;
  }
  if (
...
function applyDispatch(
  ...
  if (
    ...
    !canContinueTransportAttempt(call, state) ||
    !validateRequestedTransportIdentity(event, call, state) ||
    !validateTransportOrdinal(event.ordinal, call.nextDispatchOrdinal, state)
  ) {
    return false;
  }
  ...
  if (
    pendingDispatch
      ? pendingDispatch.ordinal !== event.attemptOrdinal ||
        pendingDispatch.reason !== event.reason ||
        pendingDispatch.nextHopOrdinal !== event.hopOrdinal
      : event.hopOrdinal !== 1
  ) {
    return rejectTransportDispatchRelation(state);
  }
  ...
  call.nextDispatchOrdinal += 1;
  call.unsettledDispatches += 1;
  call.pendingDispatchAttempt = {
    ordinal: event.attemptOrdinal,
    reason: event.reason,
    nextHopOrdinal: event.hopOrdinal + 1,
  };
  retainProviderTransportDispatch(event, call.ordinal, state, MAX_MODEL_TRANSPORT_DISPATCHES);
  return true;
}
...
function applyTransportEvent(
  ...
  switch (event.type) {
    case "dispatch":
      return applyDispatch(event, state);
    case "attempt":
