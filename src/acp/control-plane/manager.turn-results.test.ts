...
describe("AcpSessionManager turn results", () => {
  ...
  it("does not let a stalled prompt observer delay the terminal result", async () => {
    ...
    await vi.waitFor(() => expect(order).toEqual(["lifecycle", "accounting-started", "settled"]));
    await run;
    expect(order).toEqual(["lifecycle", "accounting-started", "settled"]);

    acquisitionObserved.resolve();
    await vi.waitFor(() =>
      expect(order).toEqual(["lifecycle", "accounting-started", "settled", "accounting-finished"]),
    );
  });
    ...
    for (const message of ["acpx exited with code 1", "acpx exited with signal SIGTERM"]) {
      ...
      const startTurn = vi
        ...
        .mockImplementationOnce((input: AcpRuntimeTurnInput) => ({
          requestId: input.requestId,
          promptStarted: Promise.reject(new Error("not submitted")),
          events: (async function* () {})(),
          result: Promise.resolve({
            status: "failed" as const,
  ...
  it.each([
    {
      name: "missing",
      promptStarted: undefined,
    },
    {
      name: "resolved",
      promptStarted: Promise.resolve(),
    },
  ])("does not retry after $name prompt readiness", async ({ promptStarted }) => {
    const runtimeState = createRuntime();
    const startTurn = vi.fn((input: AcpRuntimeTurnInput) => ({
      requestId: input.requestId,
    ...
    await expect(
      manager.runTurn({
        ...
        mode: "prompt",
        requestId: `run-${promptStarted ? "submitted" : "unknown"}`,
      }),
  ...
  it("does not report acquisition or retry when startTurn throws synchronously", async () => {
    const runtimeState = createRuntime();
