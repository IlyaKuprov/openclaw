...
import type { GatewayRequestContext, SessionMutationAuthorization } from "./types.js";
...
function authorizeArchiveTarget(params: {
  authorizationMode: "request" | "target";
  authorization?: SessionMutationAuthorization;
  inputTarget: { agentId?: string; key: string };
}): ErrorShape | undefined {
  try {
    if (params.authorizationMode === "request") {
      params.authorization?.assertCurrent();
    } else {
      params.authorization?.assertTargetCurrent({
        sessionKey: params.inputTarget.key,
        ...(params.inputTarget.agentId ? { agentId: params.inputTarget.agentId } : {}),
      });
    }
    return undefined;
  } catch (error) {
...
export async function prepareSessionPatchArchive(params: {
  ...
  const freshResolved = resolveGatewaySessionTargetFromKey(target.key, cfg, {
  ...
  const placementError = resolveSessionWorkerPlacementPatchError({
    agentId: freshResolved.target.agentId,
    cfg,
  ...
  const preview = await projectSessionsPatchEntry({
    ...
    isLabelInUse: (label) =>
      Object.entries(freshResolved.target.store).some(
  ...
  try {
    const drain = await prepareSessionArchiveLifecycle({
      ...
      sessionKeys: Array.from(new Set([freshCanonicalKey, target.canonicalKey, target.key])),
