...
}): Promise<Array<{ index: number; target: SessionLifecycleTarget }>> {
  if (params.pending.length === 0) {
    return [];
  }
  const archived: Array<{ index: number; target: SessionLifecycleTarget }> = [];
...
export async function runSessionLifecycleBatch(params: {
  ...
  params.items.forEach((item, index) => {
    ...
    if (params.operation === "archive" && target.archived === true) {
      results[index] = { key: target.key, ok: true, status: "already_archived" };
      return;
    }
  ...
  if (params.operation === "archive") {
    try {
      ...
      for (const { index, target } of archived) {
        results[index] = { key: target.key, ok: true, status: "archived" };
      }
    } catch (error) {
  ...
  if (params.archiveBeforeDelete) {
    const alreadyArchived = pending.filter(({ target }) => target.archived === true);
    const needArchive = pending.filter(({ target }) => target.archived !== true);
    try {
      ...
      deleteTargets = [...alreadyArchived, ...newlyArchived].toSorted(
        (left, right) => left.index - right.index,
      );
    } catch (error) {
      for (const { index, target } of needArchive) {
        results[index] ??= {
          key: target.key,
          ok: false,
          status: "failed",
